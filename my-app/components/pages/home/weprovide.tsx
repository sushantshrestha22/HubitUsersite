      ...
      <div>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          {courseData.map((course, index) => (
            <div
              key={index}
              className="bg-white p-4 rounded-lg shadow flex flex-col items-center justify-center space-y-2 hover:border-2 border-primary box-border h-52"
            >
              <MdModelTraining className="text-primary h-10 w-10"/>
              <div className="text-medium font-semibold">{course.title}</div>
              <div className="text-xs lg:text-sm text-center ">{course.description}</div>
            </div>
          ))}
        </div>
      </div>
